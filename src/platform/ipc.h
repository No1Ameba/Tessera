...
/*
 * 현재 사용자 기준 소켓 경로(또는 파이프 이름)를 buf 에 쓴다.
 *   POSIX   : "$XDG_RUNTIME_DIR/tessera-<uid>.sock" (없으면 "/tmp/...")
 *   Windows : "\\.\pipe\tessera-<username>"
 *
 * 환경변수 `TESSERA_IPC_PATH` 가 있으면 그 값을 그대로 쓴다. 데몬과 클라이언트가
 * 같은 환경을 보므로 짝이 맞고, 사용자당 하나뿐인 기본 엔드포인트에서 분리된
 * 인스턴스를 띄울 수 있다. 테스트가 실행 중인 실제 데몬과 충돌하지 않으려면
 * 반드시 필요하다 — Named Pipe 는 같은 이름의 인스턴스를 여러 개 허용해서,
 * 격리하지 않으면 테스트 클라이언트가 진짜 데몬에 붙어 버린다.
 *
